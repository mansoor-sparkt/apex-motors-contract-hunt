    ...
    <div className="absolute inset-0 flex flex-col h-full w-full overflow-hidden">
      ...
      <div className="relative z-[2] flex min-h-0 flex-1 flex-col">
            ...
            {(() => {
              ...
              return GAME_TIMELINE.map((item, i) => {
                ...
                if (isStop) {
                  ...
                  ptsLabel = done
                    ? `${stopsDone[item.index as number]?.bonus ? 20 : 10} / 20 PTS`
                    : "20 PTS MAX";
                } else {
        ...
        <CongratulationModal
          isOpen={showCongratulation}
          onClose={() => {
            // 1. Close the Congrats Modal
            setShowCongratulation(false);

            // 2. NOW check if we need to show the Incomplete Bonus Nudge
            ...
            if (wonExtraPrize || allBonusesDone) {
              // They have enough bonus points! Skip the nudge and go to the final profile.
              onTabChange("comp"); // (or onTabChange("comp") depending on how it's passed)
            } else {
              // They missed bonuses! Show the Nudge NOW.
